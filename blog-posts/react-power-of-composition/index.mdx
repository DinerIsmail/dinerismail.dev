...
Yikes! Luckily that's just made up, and in reality we can actually use the `select` element like this:
...
- Using the top-level Children API (like `Children.map`, or `Children.forEach`)
